import type { PaginatedDocs, PayloadRequest, Sort, User, Where } from 'createo'
...
export const createExport = async (args: CreateExportArgs) => {
  ...
  const collectionConfig = payload.config.collections.find(
    ({ slug }: { slug: any }) => slug === collectionSlug,
  )
  if (!collectionConfig) {
    ...
    const stream = new Readable({
      async read() {
        ...
        while (result.docs.length > 0) {
          const csvInput = result.docs.map((doc: any) => flattenObject(doc))
  ...
  while (result.hasNextPage) {
    ...
    if (isCSV) {
      ...
      const jsonInput = result.docs.map((doc: any) => JSON.stringify(doc))
